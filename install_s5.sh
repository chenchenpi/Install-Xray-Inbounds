这是基于你提供的脚本修改后的 SOCKS5 版本。
⚠️ 重要安全提示（必读）
在运行之前，请务必了解：标准 SOCKS5 协议的流量是明文（或仅弱加密）的，且特征非常明显。

无混淆/无强加密：不像 Shadowsocks 或 VMess，SOCKS5 的数据包很容易被防火墙（如 GFW）识别并阻断。
安全性：虽然脚本开启了账号密码认证，防止被他人盗用，但传输的内容（你访问的网站数据）在传输路径上是可见的。
适用场景：适合作为内网穿透的中转，或者在已经建立好的加密隧道（如 WireGuard/OpenVPN）内部使用，不建议直接在公网环境用于科学上网（极易被封 IP）。


🛠️ 修改版脚本：Xray SOCKS5 安装脚本
主要变动：

协议变更：shadowsocks -> socks。
认证方式：移除了加密算法选择（AES/ChaCha20），改为 用户名 + 密码 认证（Xray 的 SOCKS 入站强制要求认证或限制 IP，否则会被扫爆）。
链接格式：输出 socks5://user:pass@ip:port 格式的链接。
